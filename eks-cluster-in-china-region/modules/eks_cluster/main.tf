...
module "eks" {
  ...
  cluster_iam_role_dns_suffix = local.cluster_iam_role_dns_suffix
  ...
  aws_auth_roles = concat(
    [
      ...
      {
        rolearn  = "arn:aws-cn:iam::${data.aws_caller_identity.current.account_id}:role/${local.eks_admin_role_name}" # The ARN of the IAM role
